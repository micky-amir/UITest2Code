    ...
    with open(project_path + tok_file_path, 'r', encoding="utf-8") as tok_file, open(
        ...
        while line:
            ...
            if website_name == 'bigquery':
                ...
                current_test_simplified = get_some_fields_from_dictionary(current_test, ['name', 'comments'])
                # current_test_simplified = bigquery_handler.get_dictionary_of_tokenized_methods(test_id)
            else:
                test_id = test_id.rsplit('_', 1)[0].upper()
                # gets the current test from the dictionary
...
def get_some_fields_from_dictionary(dictionary, list_of_fields):
    new_dict = {key: dictionary[key] for key in dictionary.keys() & list_of_fields}
    return new_dict
